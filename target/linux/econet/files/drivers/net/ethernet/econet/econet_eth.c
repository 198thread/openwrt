...
struct en75_eth_pvt {
	...
	/* MMIO-backed MDIO bus for internal MT7530 switch */
...
/*
 * MMIO-backed MDIO bus for the internal MT7530 switch (MCM).
 *
 * MT7530 uses a paged MDIO protocol (mt7530-mdio.c lines 18-63):
 *   - write page: bus write to addr=0x1f, reg=0x1f, val=page
 *   - read lo16:  bus read  addr=0x1f, reg=r  → switch_regs[page*0x10 + r] & 0xffff
 *   - read hi16:  bus read  addr=0x1f, reg=0x10 → same MMIO word >> 16
 *   - write lo16: bus write addr=0x1f, reg=r  → preserve hi16
 *   - write hi16: bus write addr=0x1f, reg=0x10 → preserve lo16
 *
 * Each switch register is 32-bit at switch_regs[page*0x10 + r].
 * (page offset = page * 0x40 bytes = page * 0x10 u32-words)
 *
 * Only addr=0x1f is the switch itself.  Internal PHYs (addr 0-4) are
 * not implemented here; they are handled by mt7530.c via the switch
 * register space, not via direct MDIO.
 */
static int en75_sw_mdio_read(struct mii_bus *bus, int addr, int regnum)
{
	struct en75_eth_pvt *eth = bus->priv;
	u32 __iomem *sw = eth->regs->switch_regs;
	u32 word;

	if (addr != 0x1f)
		return 0xffff;

	/* Virtual page register — return cached value */
	if (regnum == 0x1f)
		return eth->sw_page;

	/* Hi16 read: return upper 16 bits of the last lo16 register */
	if (regnum == 0x10) {
		word = readl(&sw[eth->sw_page * 0x10 + eth->sw_last_lo_reg]);
		return (word >> 16) & 0xffff;
	}

	/* Normal lo16 read */
	eth->sw_last_lo_reg = regnum;
	word = readl(&sw[eth->sw_page * 0x10 + regnum]);
	return word & 0xffff;
}

static int en75_sw_mdio_read_c45(struct mii_bus *bus, int addr, int devad,
				  int regnum)
{
	return 0xffff;
}

static int en75_sw_mdio_write_c45(struct mii_bus *bus, int addr, int devad,
				   int regnum, u16 val)
{
	return 0;
}

static int en75_sw_mdio_write(struct mii_bus *bus, int addr, int regnum, u16 val)
{
	struct en75_eth_pvt *eth = bus->priv;
	u32 __iomem *sw = eth->regs->switch_regs;
	u32 word;
	...
	/* Hi16 write: preserve lo16 */
	if (regnum == 0x10) {
		word = readl(&sw[eth->sw_page * 0x10 + eth->sw_last_lo_reg]);
		writel(((u32)val << 16) | (word & 0xffff),
		       &sw[eth->sw_page * 0x10 + eth->sw_last_lo_reg]);
		return 0;
	}

	/* Lo16 write: preserve hi16 */
	eth->sw_last_lo_reg = regnum;
	word = readl(&sw[eth->sw_page * 0x10 + regnum]);
	writel((word & 0xffff0000) | val, &sw[eth->sw_page * 0x10 + regnum]);
	return 0;
}

static int en75_sw_mdio_init(struct en75_eth_pvt *eth, struct device_node *eth_np)
{
	struct device *dev = eth->pub.dev;
	struct device_node *mdio_np;
	struct mii_bus *bus;
	int ret;

	mdio_np = of_get_child_by_name(eth_np, "mdio-bus");
	if (!mdio_np)
		return 0;
	...
	bus->name = "econet-sw-mdio";
	snprintf(bus->id, MII_BUS_ID_SIZE, "%s", dev_name(dev));
	bus->read = en75_sw_mdio_read;
	bus->write = en75_sw_mdio_write;
	bus->read_c45 = en75_sw_mdio_read_c45;
	bus->write_c45 = en75_sw_mdio_write_c45;
	bus->priv = eth;
	bus->parent = dev;
	...
	of_node_put(mdio_np);
	if (ret) {
		dev_err(dev, "failed to register switch MDIO bus: %d\n", ret);
		return ret;
	}
...
static int en75_probe(struct platform_device *pdev)
{
	...
	/*
	 * Register MMIO-backed MDIO bus for the internal MT7530 switch.
	 * Must happen before of_platform_populate() to ensure the bus exists
	 * when child platform devices (e.g. mdio-gpio) try to bind.
	 */
	err = en75_sw_mdio_init(eth, pdev->dev.of_node);
	if (err)
		goto error;

	/* Populate any remaining platform devices for other child nodes. */
...
static const struct of_device_id of_en75_match[] = {
	...
	{ .compatible = "econet,en751627-eth", .data = &en751221_soc_data },
